...
const ShopClient = ({ products, category, page }: ShopClientProps) => {
  ...
  const renderProducts = () => {
    ...
    return productsToDisplay.map((product, index) => (
      <div
        key={index}
        className="w-[360px] h-[450px] bg-[#F6F6F6] flex flex-col items-center justify-center py-4 px-2 gap-4 shadow-inner transition-all transform hover:scale-105 hover:shadow-lg"
      >
        <Image
